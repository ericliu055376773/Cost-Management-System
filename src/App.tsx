...
const getIconByCategory = (categoryName) => {
  ...
  if (categoryName.includes('肉'))
    return <Beef className="w-6 h-6 text-black" strokeWidth={1.5} />;
  if (categoryName.includes('菜') || categoryName.includes('蔬'))
    return <Carrot className="w-6 h-6 text-black" strokeWidth={1.5} />;
  if (
    categoryName.includes('海') ||
    categoryName.includes('魚') ||
    categoryName.includes('蝦')
  )
    return <Fish className="w-6 h-6 text-black" strokeWidth={1.5} />;
  return <Coffee className="w-6 h-6 text-black" strokeWidth={1.5} />;
};
...
const defaultRules = {
  無骨雞腿肉: {
    yieldRate: 85,
    calculationText: '進貨價 ÷ 淨肉率 (85%)',
    recipeUnit: 'g',
    recipeConversion: 1000,
  },
  特級牛五花: {
    yieldRate: 90,
    calculationText: '進貨價 ÷ 淨肉率 (90%)',
    recipeUnit: 'g',
    recipeConversion: 1000,
  },
  高山高麗菜: {
    yieldRate: 70,
    calculationText: '去外葉耗損 (70%)',
    recipeUnit: 'g',
    recipeConversion: 1000,
  },
  手工蛋餃: {
    yieldRate: 100,
    calculationText: '無耗損 (一包約30個)',
    recipeUnit: '個',
    recipeConversion: 30,
  },
  '冷凍白蝦 (40/50)': {
    yieldRate: 50,
    calculationText: '進貨價 ÷ 剝殼去頭淨肉率 (50%)',
    recipeUnit: 'g',
    recipeConversion: 1000,
  },
};
...
export default function App() {
  ...
  const [editingGlobalSettings, setEditingGlobalSettings] = useState(
  ...
  const [tempSystemName, setTempSystemName] = useState(
    defaultGlobalSettings.systemName
  );
  ...
  const erpData = useMemo(() => {
    ...
    rawErpData.forEach((item) => {
      ...
      if (!grouped[item.category][item.vendor])
        grouped[item.category][item.vendor] = [];

      const rule = productRules[item.name] || {
        yieldRate: 100,
        calculationText: '進貨價 ÷ 100% (無耗損)',
  ...
  const availableCategories = useMemo(() => {
    const cats = new Set(
      uniqueProductsForSettings.map((item) => item.category)
    );
  ...
  const filteredSettingsProducts = useMemo(() => {
    return uniqueProductsForSettings.filter((item) => {
      const matchCat =
        settingsCategoryFilter === '全部' ||
        item.category === settingsCategoryFilter;
      ...
      try {
        ...
        onSnapshot(rulesRef, (docSnap) => {
          if (docSnap.exists()) {
            ...
            if (data.rules)
              setProductRules((prev) => ({ ...prev, ...data.rules }));
  ...
  const saveSettingsToCloud = async () => {
    ...
    try {
      ...
      await setDoc(
        rulesRef,
        { rules: editingRules, globalSettings: editingGlobalSettings },
        { merge: true }
      );
  ...
  const handleRuleChange = (productName, field, value) => {
    setEditingRules((prev) => {
      const existingRule = prev[productName] || {
        yieldRate: 100,
        calculationText: '進貨價 ÷ 100% (無耗損)',
        recipeUnit: '',
        recipeConversion: 1,
  ...
  const renderUIEditor = () => {
    ...
    const renderInputField = (label, valueKey) => (
      <div key={valueKey}>
        <label className="block text-xs font-bold text-[#7F7F7F] mb-2 tracking-widest uppercase">
          {label}
        </label>
        <input
          type="text"
          value={tempUITexts[valueKey] || ''}
          onChange={(e) =>
            setTempUITexts({ ...tempUITexts, [valueKey]: e.target.value })
          }
      ...
      <div className="fixed inset-0 z-[100] bg-[#F5F5F5] flex flex-col animate-in fade-in duration-300 overflow-y-auto selection:bg-neutral-200">
        <div className="bg-white border-b border-neutral-100 sticky top-0 z-10 px-6 py-4 flex justify-between items-center shadow-[0_10px_30px_rgba(0,0,0,0.02)]">
          <div className="flex items-center gap-4">
            <button
              onClick={() => setIsEditingUI(false)}
              className="w-10 h-10 flex items-center justify-center bg-[#F5F5F5] rounded-full text-black hover:bg-neutral-200 transition-colors"
            >
              <ChevronLeft size={18} />
            </button>
          ...
          <button
            onClick={saveUITexts}
            className="bg-black hover:bg-neutral-800 text-white px-6 py-2.5 rounded-full font-bold text-sm shadow-[0_10px_20px_rgba(0,0,0,0.08)] transition-all"
          >
            儲存變更
          </button>
        </div>
  ...
  if (!isAuthenticated) {
    return (
      <div className="min-h-screen bg-white flex flex-col items-center justify-center p-6 font-sans selection:bg-neutral-200 relative overflow-hidden">
        ...
        {/* 簡約科技感登入區塊 (白底版) */}
        <div
          className={`w-full max-w-[320px] flex flex-col items-center z-10 ${
        ...
        >
          ...
          <h1 className="text-3xl font-semibold text-black tracking-wide mb-12">
            {texts.loginTitle}
          </h1>
          ...
          <div className="w-full">
            <form onSubmit={handleLogin} className="flex flex-col items-center">
                  ...
                  return (
                    <div
                      ...
                      className={`w-3.5 h-3.5 rounded-full transition-all duration-300 ease-out ${
                        isFilled
                          ? 'bg-black scale-125 shadow-[0_4px_10px_rgba(0,0,0,0.15)]'
                          : 'bg-[#E5E5E5]'
                      }`}
                    />
                  );
              ...
              <div className="h-6 mb-8 flex items-center justify-center w-full">
                {(pin.length < 4 || error) && (
                  <p className="text-[#E04D41] text-xs font-bold tracking-wide">
                    {error || texts.loginErrorNotLongEnough}
                  </p>
                )}
              </div>
      ...
      <div className="animate-in fade-in duration-500 pb-24 md:pb-8">
        <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4 mb-8">
          <div>
            ...
            <p className="text-[#7F7F7F] font-medium text-sm mt-2 tracking-wide">
              設定全域稅率、顯示名稱與各項食材規則。
            </p>
          </div>
          <button
        ...
        <div className="bg-white p-8 rounded-[32px] shadow-[0_10px_30px_rgba(0,0,0,0.03)] border border-neutral-100 mb-8">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 max-w-2xl">
            <div>
              <label className="block text-xs font-bold text-[#7F7F7F] mb-3 tracking-widest uppercase">
                系統顯示名稱 (LOGO)
              </label>
              <div className="relative">
                <input
                  ...
                  onChange={(e) =>
                    setEditingGlobalSettings({
                      ...editingGlobalSettings,
              ...
              <button
                ...
                className={`px-5 py-2.5 rounded-full font-bold text-sm transition-all ${
                  settingsCategoryFilter === cat
                    ? 'bg-black text-white'
                    : 'bg-[#F5F5F5] text-[#7F7F7F] hover:bg-neutral-200 hover:text-black'
                }`}
              >
                {cat}
              </button>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {filteredSettingsProducts.map((item, idx) => {
            const rule = editingRules[item.name] || {
              yieldRate: 100,
              calculationText: '進貨價 ÷ 100% (無耗損)',
              recipeUnit: item.unit,
              recipeConversion: 1,
            ...
            return (
              <div
                key={idx}
                className="bg-white rounded-[32px] p-8 shadow-[0_10px_30px_rgba(0,0,0,0.03)] border border-neutral-100 flex flex-col"
              >
                <div className="flex justify-between items-start mb-6">
                  <div>
                    <span className="text-[10px] font-bold text-[#7F7F7F] uppercase tracking-widest block mb-2">
                      {item.category}
                    </span>
                  ...
                  <div className="text-right">
                    <span className="text-[10px] text-[#7F7F7F] font-bold tracking-widest uppercase block mb-1">
                      進貨參考價
                    </span>
                    <span className="font-black text-black text-lg">
                      ${item.rawPrice}{' '}
                      <span className="text-xs text-[#7F7F7F] font-medium">
                        /{item.unit}
                      </span>
                    </span>
                  </div>
                ...
                <div className="space-y-6 flex-1">
                  ...
                  <div className="grid grid-cols-2 gap-4">
                    ...
                    <div>
                      <label
                        className="block text-xs font-bold text-[#7F7F7F] mb-3 tracking-widest uppercase truncate"
                        title={`1 ${item.unit} = ? ${
                          rule.recipeUnit || item.unit
                        }`}
                      >
                        單位轉換
                      </label>
                ...
                <div className="mt-8 pt-6 border-t border-neutral-100 flex justify-between items-center">
                  ...
                  <span className="font-black text-black text-2xl flex items-center">
                    ...
                    {previewRecipeCost < 1
                      ? previewRecipeCost.toFixed(4)
                      : previewRecipeCost.toFixed(2)}
                    <span className="text-sm opacity-50 font-medium ml-1">
                      /{rUnit}
                    </span>
                  </span>
                </div>
    ...
    if (editingMenu) {
      ...
      const updatedCategories = (editingMenu.categories || []).map((cat) => {
        const updatedItems = (cat.items || []).map((ig) => {
          const latestInfo =
            allAvailableIngredients.find(
              (a) => a.name === ig.itemName && a.vendorName === ig.vendorName
            ) || ig;
          ...
          return {
            ...
            recipeCost: rCost,
            recipeUnit: rUnit,
            finalCost: latestInfo.finalCost,
          };
        });
      ...
      const taxAmount =
        (editingMenu.sellingPrice || 0) * (globalSettings.taxRate / 100);
      const totalCost = totalIngredientsCost + taxAmount;
      const margin =
        editingMenu.sellingPrice > 0
          ? (
              ((editingMenu.sellingPrice - totalCost) /
                editingMenu.sellingPrice) *
              100
            ).toFixed(1)
          : 0;
      ...
      const handleAddCategory = () =>
        setEditingMenu({
          ...
          categories: [
            ...updatedCategories,
            { id: `cat-${Date.now()}`, name: '未命名分類', items: [] },
          ],
        });
      ...
      const handleRemoveCategory = (catId) =>
        setEditingMenu({
          ...editingMenu,
          categories: updatedCategories.filter((c) => c.id !== catId),
        });
      ...
      const handleUpdateIngredientQty = (catId, igId, newQty) =>
        setEditingMenu({
          ...editingMenu,
          categories: updatedCategories.map((c) =>
            c.id === catId
              ? {
                  ...c,
                  items: c.items.map((i) =>
                    i.id === igId ? { ...i, qty: newQty } : i
                  ),
                }
              : c
          ),
        });
      const handleRemoveIngredient = (catId, igId) =>
        setEditingMenu({
          ...editingMenu,
          categories: updatedCategories.map((c) =>
            c.id === catId
              ? { ...c, items: c.items.filter((i) => i.id !== igId) }
              : c
          ),
        });
      const handleSaveMenu = () => {
        const menuToSave = { ...editingMenu, categories: updatedCategories };
        if (setMenus.find((m) => m.id === editingMenu.id))
          setSetMenus(
            setMenus.map((m) => (m.id === editingMenu.id ? menuToSave : m))
          );
      ...
      return (
        <div className="animate-in fade-in slide-in-from-right-4 duration-300 pb-24 md:pb-8">
          <button
            ...
            className="flex items-center text-[#7F7F7F] hover:text-black font-bold mb-8 transition-all w-fit"
          >
            <div className="w-8 h-8 rounded-full bg-white shadow-sm flex items-center justify-center mr-3 border border-neutral-100">
          ...
          <div className="bg-white rounded-[32px] shadow-[0_10px_30px_rgba(0,0,0,0.03)] border border-neutral-100 overflow-hidden mb-8 flex flex-col">
            ...
            <div className="bg-black p-8 flex flex-col md:flex-row md:items-center justify-between gap-6">
              <div className="flex-1 w-full">
                <span className="text-[#7F7F7F] text-xs font-bold uppercase tracking-widest mb-2 block">
                  商品名稱
                </span>
                <input
                  type="text"
              ...
              <div className="bg-neutral-900 p-5 rounded-[24px] border border-neutral-800 flex items-center gap-4 shrink-0">
                <span className="text-[#7F7F7F] font-bold text-xs uppercase tracking-widest">
                  終端售價
                </span>
                <div className="flex items-center text-white">
                  ...
                  <input
                    type="number"
                    value={editingMenu.sellingPrice}
                    onChange={(e) =>
                      setEditingMenu({
                        ...editingMenu,
            ...
            <div className="p-6 md:p-8 bg-white flex-1">
              <div className="flex justify-between items-end mb-8">
                ...
                <button
                  ...
                  className="flex items-center gap-2 bg-[#F5F5F5] hover:bg-neutral-200 text-black px-5 py-3 rounded-full font-bold text-sm transition-all whitespace-nowrap"
                >
              ...
              <div className="space-y-8">
                {updatedCategories.map((cat) => {
                  const catSubtotal = (cat.items || []).reduce(
                    (sum, ig) => sum + (ig.recipeCost || 0) * ig.qty,
                    0
                  );
                  return (
                    <div
                      key={cat.id}
                      className="bg-[#F5F5F5] rounded-[24px] p-6"
                    >
                      <div className="flex flex-col xl:flex-row xl:items-center justify-between gap-4 mb-6">
                        ...
                        <div className="flex items-center gap-3 xl:justify-end">
                          <div className="flex items-center bg-white p-1.5 rounded-full shadow-sm">
                            <select
                              value={ingredientCategoryFilters[cat.id] || ''}
                              onChange={(e) =>
                            ...
                            >
                              ...
                              {allAvailableIngredients
                                .filter((ing) =>
                                  ingredientCategoryFilters[cat.id]
                                    ? ing.categoryName ===
                                      ingredientCategoryFilters[cat.id]
                                    : true
                                )
                          ...
                          <button
                            ...
                            className="w-10 h-10 flex items-center justify-center text-neutral-400 hover:text-black bg-white rounded-full shadow-sm transition-colors"
                          >
                            <Trash2 size={16} />
                          </button>
                        </div>
                      ...
                      <div className="space-y-3">
                        {cat.items.map((ig) => (
                          <div
                            key={ig.id}
                            className="flex flex-col sm:flex-row sm:items-center justify-between bg-white p-4 rounded-[16px] gap-4 shadow-sm border border-transparent hover:border-neutral-200 transition-colors"
                          >
                            <div className="flex-1 flex items-center gap-4">
                              ...
                              <div>
                                <h4 className="font-black text-black text-base leading-none mb-1.5">
                                  {ig.itemName}
                                </h4>
                                <p className="text-[10px] text-[#7F7F7F] font-bold uppercase tracking-widest">
                                  {ig.vendorName} • $
                                  {(ig.recipeCost || 0) < 1
                                    ? (ig.recipeCost || 0).toFixed(4)
                                    : (ig.recipeCost || 0).toFixed(2)}
                                  /{ig.recipeUnit}
                                </p>
                              </div>
                            </div>
                            ...
                            <div className="flex items-center gap-6 sm:justify-end">
                              <div className="flex items-center bg-[#F5F5F5] rounded-full overflow-hidden focus-within:ring-2 focus-within:ring-black">
                                ...
                                />
                                <span className="pr-4 text-[#7F7F7F] font-bold text-xs">
                                  {ig.recipeUnit}
                                </span>
                              </div>
                              ...
                              <div className="w-20 text-right">
                                <span className="font-black text-black text-lg">
                                  ${((ig.recipeCost || 0) * ig.qty).toFixed(1)}
                                </span>
                              </div>

                              <button
                                onClick={() =>
                                  handleRemoveIngredient(cat.id, ig.id)
                                }
                ...
                {updatedCategories.length === 0 && (
                  <div className="text-center py-20 bg-[#F5F5F5] rounded-[24px]">
                    ...
                    <p className="text-black font-black text-xl mb-4 tracking-tight">
                      尚未建立任何分類
                    </p>
                    <button
                      onClick={handleAddCategory}
                      className="bg-black text-white px-6 py-3 rounded-full font-bold transition-all shadow-[0_10px_20px_rgba(0,0,0,0.08)] hover:bg-neutral-800"
                    >
                      新增第一個分類
                    </button>
                  </div>
            ...
            <div className="bg-[#F5F5F5] p-6 md:p-8 flex flex-col xl:flex-row justify-between items-center gap-6 rounded-b-[32px]">
              <div className="flex gap-6 sm:gap-10 w-full xl:w-auto flex-wrap">
                <div>
                  <span className="block text-[10px] text-[#7F7F7F] font-bold uppercase tracking-widest mb-1">
                    食材總成本
                  </span>
                  <span className="text-2xl font-black text-black">
                    ${totalIngredientsCost.toFixed(1)}
                  </span>
                </div>
                <div>
                  <span className="block text-[10px] text-[#7F7F7F] font-bold uppercase tracking-widest mb-1">
                    營業稅 ({globalSettings.taxRate}%)
                  </span>
                  <span className="text-2xl font-black text-black">
                    ${taxAmount.toFixed(1)}
                  </span>
              ...
              <button
                ...
                className="w-full xl:w-auto bg-black hover:bg-neutral-800 text-white px-10 py-4 rounded-full font-bold text-lg shadow-[0_10px_20px_rgba(0,0,0,0.1)] transition-all flex items-center justify-center gap-2"
              >
                儲存商品配方
              </button>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {setMenus.map((menu) => {
            ...
            (menu.categories || []).forEach((cat) => {
              (cat.items || []).forEach((ig) => {
                const latestInfo =
                  allAvailableIngredients.find(
                    (a) =>
                      a.name === ig.itemName && a.vendorName === ig.vendorName
                  ) || ig;
                totalIngredientsCost +=
                  (latestInfo.recipeCost || latestInfo.finalCost || 0) * ig.qty;
                totalItemsCount += 1;
              });
            });
            const taxAmount =
              (menu.sellingPrice || 0) * (globalSettings.taxRate / 100);
            const totalCost = totalIngredientsCost + taxAmount;
            const margin =
              menu.sellingPrice > 0
                ? (
                    ((menu.sellingPrice - totalCost) / menu.sellingPrice) *
                    100
                  ).toFixed(1)
                : 0;
            ...
            return (
              <div
                key={menu.id}
                className="bg-white rounded-[32px] p-8 shadow-[0_10px_30px_rgba(0,0,0,0.03)] border border-neutral-100 flex flex-col hover:shadow-[0_20px_40px_rgba(0,0,0,0.06)] transition-all group"
              >
                <div className="flex justify-between items-start mb-6">
                  ...
                  <button
                    ...
                    className="w-10 h-10 flex items-center justify-center text-neutral-400 hover:text-black bg-[#F5F5F5] rounded-full transition-colors"
                  >
                    <Edit2 size={16} />
                  </button>
                </div>
                ...
                <div className="flex items-center gap-2 mb-8 border-b border-neutral-100 pb-6">
                  <span className="bg-[#F5F5F5] text-[#7F7F7F] text-xs font-bold px-3 py-1 rounded-full uppercase tracking-widest">
                    {(menu.categories || []).length} 個分類
                  </span>
                ...
                <div className="mt-auto space-y-5">
                  <div className="flex justify-between items-center">
                    <span className="text-[10px] text-[#7F7F7F] font-bold uppercase tracking-widest">
                      終端售價
                    </span>
                    <span className="font-black text-black text-lg">
                      ${menu.sellingPrice}
      ...
      return (
        <div className="pb-24 md:pb-8">
          <h2 className="text-[#7F7F7F] text-sm font-bold mb-6 flex items-center gap-2 uppercase tracking-widest">
            <Search className="w-4 h-4" /> 搜尋結果： "{searchQuery}" (
        ...
        <div className="animate-in fade-in slide-in-from-right-4 duration-300 pb-24 md:pb-8">
          <div className="flex items-center gap-3 mb-8 text-xs font-bold uppercase tracking-widest">
            <button
              ...
              className="text-[#7F7F7F] hover:text-black transition-colors"
            >
              分類總覽
            </button>
          ...
          <div className="flex items-center mb-10 bg-white p-8 rounded-[32px] shadow-[0_10px_30px_rgba(0,0,0,0.03)] border border-neutral-100">
            <div className="w-16 h-16 bg-[#F5F5F5] rounded-full flex items-center justify-center mr-6">
              <Store className="w-6 h-6 text-black" strokeWidth={1.5} />
            </div>
            <div>
              <h2 className="text-3xl font-black text-black tracking-tight">
                {selectedVendor.vendorName}
              </h2>
              <p className="text-sm font-bold text-[#7F7F7F] mt-1">
                提供 {selectedVendor.items.length} 項供應食材
              </p>
            </div>
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
      ...
      return (
        <div className="animate-in fade-in slide-in-from-right-4 duration-300 pb-24 md:pb-8">
          ...
          <div className="flex items-center mb-10">
            <div
              className={`w-20 h-20 rounded-full flex items-center justify-center bg-white shadow-[0_10px_30px_rgba(0,0,0,0.05)] border border-neutral-100 mr-6`}
            >
            ...
            <div>
              ...
              <p className="text-sm font-bold text-[#7F7F7F] mt-1 uppercase tracking-widest">
                請選擇進貨廠商
              </p>
            </div>
          </div>
            ...
            {selectedCategory.vendors.map((vendor, idx) => (
              ...
              >
                <div className="flex items-start justify-between mb-8">
                  <div className="w-12 h-12 bg-[#F5F5F5] rounded-full flex items-center justify-center group-hover:bg-black transition-colors">
                    <Store
                      className="w-5 h-5 text-black group-hover:text-white"
                      strokeWidth={1.5}
                    />
                  </div>
                  <div className="bg-[#F5F5F5] text-[#7F7F7F] text-[10px] font-bold px-3 py-1.5 rounded-full uppercase tracking-widest">
                    {selectedCategory.category}
                  </div>
                </div>
                <div>
                  ...
                  <p className="text-sm text-[#7F7F7F] font-bold pt-4 flex items-center justify-between">
                    {vendor.items.length} 項食材{' '}
                    <ChevronRight
                      size={16}
                      className="text-neutral-300 group-hover:translate-x-1 group-hover:text-black transition-all"
                    />
                  </p>
                </div>
              </button>
      ...
      <div className="animate-in fade-in duration-500 pb-24 md:pb-8">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 sm:gap-6">
          {erpData.map((category) => (
            ...
            >
              <div
                className={`w-16 h-16 rounded-full flex items-center justify-center mb-6 group-hover:scale-110 transition-transform duration-300 bg-[#F5F5F5]`}
              >
              ...
              <p className="text-[10px] text-[#7F7F7F] font-bold mt-3 bg-white px-3 py-1 rounded-full uppercase tracking-widest border border-neutral-100 shadow-sm">
                {category.vendors.length} 家廠商
              </p>
            </button>
          ))}
        </div>
  ...
  const navItems = [
    {
      ...
      label: texts.tabIngredients,
    },
    {
      id: 'setMenus',
      icon: <Utensils size={20} strokeWidth={2} />,
      label: texts.tabSetMenus,
    },
    {
      id: 'settings',
      icon: <Settings size={20} strokeWidth={2} />,
      label: texts.tabSettings,
    },
  ];
  ...
  return (
    <div className="min-h-screen bg-[#F5F5F5] text-black flex font-sans selection:bg-neutral-200">
      ...
      <aside className="hidden md:flex flex-col w-[280px] bg-white fixed h-full z-40 border-r border-neutral-100 shadow-[10px_0_30px_rgba(0,0,0,0.02)]">
        {/* 電腦版左上角 LOGO (可點擊修改文字) */}
        <button
          onClick={openUIEditor}
          className="p-8 flex items-center gap-4 hover:bg-neutral-50 transition-colors w-full text-left group"
        >
          <div className="w-10 h-10 bg-black rounded-[14px] flex items-center justify-center shadow-md group-hover:scale-105 transition-transform shrink-0">
            <Calculator className="w-5 h-5 text-white" strokeWidth={2} />
          </div>
          <div className="flex flex-col">
            <h1 className="text-xl font-black text-black tracking-tighter uppercase truncate">
              {globalSettings.systemName || 'FINERP'}
            </h1>
            <span className="text-[9px] font-bold text-neutral-400 uppercase tracking-widest mt-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
              點擊修改設定
            </span>
          </div>
        </button>
        ...
        <nav className="flex-1 px-6 space-y-3 mt-4">
          <p className="px-2 text-[10px] font-bold text-[#7F7F7F] uppercase tracking-[0.2em] mb-4">
            {texts.sidebarMenu}
          </p>
          {navItems.map((item) => (
            <button
        ...
        <div className="p-6">
          <div className="bg-[#F5F5F5] p-5 rounded-[24px]">
            <div className="text-[10px] text-[#7F7F7F] font-bold uppercase tracking-widest mb-2">
              {texts.statusTitle}
            </div>
        ...
        <header className="bg-[#F5F5F5]/80 backdrop-blur-xl sticky top-0 z-30 px-4 py-4 md:px-10 md:py-8">
          <div className="max-w-6xl mx-auto flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            ...
            <div className="flex md:hidden justify-between items-center w-full bg-white p-4 rounded-[24px] shadow-[0_10px_30px_rgba(0,0,0,0.03)] border border-neutral-100">
              ...
              <button
                onClick={openUIEditor}
                className="flex items-center gap-3 w-fit text-left active:scale-95 transition-transform group"
              >
                <div className="w-8 h-8 bg-black rounded-[10px] flex items-center justify-center shrink-0">
                  <Calculator className="w-4 h-4 text-white" />
                </div>
                <div className="flex flex-col">
                  <h1 className="text-lg font-black text-black tracking-tighter uppercase truncate max-w-[150px]">
                    {globalSettings.systemName || 'FINERP'}
                  </h1>
                  <span className="text-[8px] text-neutral-400 font-bold block -mt-0.5 leading-none uppercase">
                    介面文字設定
                  </span>
                </div>
              </button>
              <button
                onClick={handleSync}
                disabled={isSyncing}
                className={`w-10 h-10 flex items-center justify-center rounded-full bg-[#F5F5F5] text-black ${
          ...
          {syncMessage && (
            <div
              className={`mt-4 p-4 rounded-[20px] flex items-center text-sm font-bold max-w-6xl mx-auto shadow-sm
              ${
        ...
        <div className="flex justify-around items-center p-2">
          {navItems.map((item) => (
            <button
              ...
              className={`flex flex-col items-center justify-center transition-all w-full py-3 px-2 rounded-[24px] ${
                currentTab === item.id
                  ? 'bg-white text-black'
                  : 'text-neutral-500'
              }`}
    ...
    <div className="bg-white rounded-[32px] border border-neutral-100 shadow-[0_10px_30px_rgba(0,0,0,0.03)] hover:shadow-[0_20px_40px_rgba(0,0,0,0.06)] hover:border-black transition-all overflow-hidden flex flex-col group">
      <div className="p-6 md:p-8 border-b border-neutral-100 flex flex-col gap-4">
        <div className="flex gap-2">
          {categoryName && (
            <span className="text-[10px] font-bold text-[#7F7F7F] bg-[#F5F5F5] px-3 py-1.5 rounded-full uppercase tracking-widest">
              {categoryName}
            </span>
          )}
          {vendorName && (
            <span className="text-[10px] font-bold text-black bg-white border border-neutral-200 px-3 py-1.5 rounded-full uppercase tracking-widest shadow-sm">
              {vendorName}
            </span>
          )}
        </div>
      ...
      <div className="p-6 md:p-8 flex-grow flex flex-col space-y-6">
        <div className="flex justify-between items-center border-b border-neutral-100 pb-4">
          <span className="text-[#7F7F7F] font-bold uppercase tracking-widest text-[10px]">
            最新進貨單價
          </span>
        ...
        <div className="bg-[#F5F5F5] rounded-[20px] p-5">
          <span className="block text-[#7F7F7F] font-bold mb-2 text-[10px] uppercase tracking-widest">
            成本算法
          </span>
        ...
        <div className="pt-2 mt-auto flex justify-between items-end">
          <span className="text-[#7F7F7F] text-[10px] font-bold uppercase tracking-widest mb-1.5">
            精算盤點成本
          </span>
          <div className="flex items-center text-black">
            <DollarSign className="w-5 h-5 mr-0.5 opacity-40" />
            <span className="text-3xl font-black tracking-tighter">
              {typeof item.finalCost === 'number'
                ? item.finalCost.toFixed(1)
                : item.finalCost}
            </span>
            <span className="text-sm ml-1 font-medium opacity-50">
              /{item.unit}
            </span>
          </div>
        </div>
